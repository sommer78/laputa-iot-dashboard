...
export default defineComponent({
  ...
  setup() {
    ...
    const state = ref('1');
    const getOptions = ref<OptionsItem[]>([]);
    ...
    async function fetch() {
       const tents  = (await userStore.getTenantList()) as unknown as TenantModel[];
       let prev = [] as OptionsItem[];
        tents.forEach(item=>{
         prev.push({ label: item.name,
         value: JSON.stringify(item)})
      
       });
       state.value = tents[0].name;
       getOptions.value =prev;
    }

    const handleChange = (e)=>{
        // console.log('handleChange',e);
        userStore.setCurTenant(JSON.parse(e))
    }
    const handleFetch = ()=>{
    ...
    onMounted(() => {
      const curTent = userStore.getTenant;
      console.log(curTent);
